...
export class NotesService {
  ...
  async addBranches(dto:{name:string}){
    try {
     const branch = await this.prismaService.branch.create({
      ...
      if(!branch) throw new InternalServerErrorException('Failed to add branch');
  ...
  async getAllBranches() {
    try {
      ...
      throw new InternalServerErrorException('Error while fetching branches');
    }
  }
  ...
  async deleteBranch(branchId: string) {
    try {
      ...
      throw new InternalServerErrorException('Error while deleting branch');
    }
  }
